...
class OANDAService:
    """Service to handle all OANDA API interactions for USD/JPY ML Ensemble Bot"""
    ...
    def place_market_order(self, instrument: str, units: float, stop_loss: float = None, take_profit: float = None, client_tag: str = None) -> Optional[dict]:
        """Place a market order with proper price formatting for USD/JPY (3 decimals)"""
        ...
        tag_value = client_tag or "USDJPY-ML-Ensemble"
        client_extensions = {
            "tag": tag_value,
            "comment": f"USD/JPY ML Ensemble Bot - {tag_value}"
        }
        ...
        # Fixed stop loss and take profit with proper formatting (3 decimals for USD/JPY)
        if stop_loss:
    ...
    def get_open_trades(self, instrument: str = "USD_JPY") -> Optional[List[dict]]:
    ...
    def calculate_position_size(self, account_balance: float, risk_percentage: float, stop_loss_pips: float) -> float:
        """Calculate position size based on risk for USD/JPY"""
        try:
            # Risk amount in account currency
            risk_amount = account_balance * (risk_percentage / 100)
            
            # For USD/JPY, pip value is $10 per 100,000 units (standard lot)
            # Each pip move = $10 per 100k units = $0.01 per unit
            # USD/JPY pip = 0.01 (2 decimal places)
            dollar_risk_per_unit = stop_loss_pips * 0.01
