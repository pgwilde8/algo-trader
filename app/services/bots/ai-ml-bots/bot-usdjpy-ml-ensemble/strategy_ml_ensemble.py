"""
USD/JPY ML Ensemble Trading Strategy
Uses signal engine (rule-based for now, ML-ready when models are trained)
"""
...
from usdjpy_signal_engine import get_current_signal
from app.utils.simple_news_avoidance import simple_news_avoidance

logger = logging.getLogger(__name__)

class USDJPYMLEnsembleStrategy:
    """Trading strategy using signal engine (rule-based for now, ML-ready)"""
    ...
    def calculate_stop_loss_pips(self, entry_price: float, stop_loss: float) -> float:
        """Calculate stop loss in pips for USD/JPY"""
        # USD/JPY: 1 pip = 0.01 (2 decimal places)
        pips = abs(entry_price - stop_loss) / 0.01
        return pips
    ...
    def place_trade(self, signal: Dict) -> bool:
        ...
        try:
            ...
            if not atr:
                logger.warning("No ATR in signal, using default 0.50 for USD/JPY")
                atr = 0.50  # Default ATR for USD/JPY (typically 0.5-2.0)
            ...
            logger.info(f"  Entry: {current_price:.3f}")
            logger.info(f"  Stop Loss: {stop_loss:.3f} ({stop_loss_pips:.1f} pips)")
            logger.info(f"  Take Profit: {take_profit:.3f}")
    ...
    def run_cycle(self):
        ...
        try:
            logger.info("-" * 60)
            logger.info("🔄 Running USD/JPY ML Ensemble Strategy Cycle")
