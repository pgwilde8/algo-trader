...
Automated trading bot using signal engine (rule-based for now, ML-ready)
"""
...
from strategy_ml_ensemble import USDJPYMLEnsembleStrategy
...
log_file = log_dir / f"usdjpy_ml_ensemble_{datetime.now(UTC).strftime('%Y%m%d')}.log"
...
def main():
    """Main bot loop"""
    logger.info("=" * 70)
    logger.info("🤖💵💴 USD/JPY ML ENSEMBLE AUTO TRADER STARTING")
    logger.info("=" * 70)
    ...
    try:
        ...
        account_id_usdjpy = os.getenv('OANDA_ACCOUNT_ID_USDJPY')
        account_id_general = os.getenv('OANDA_ACCOUNT_ID')
        
        if account_id_usdjpy:
            logger.info(f"📋 Found OANDA_ACCOUNT_ID_USDJPY: {account_id_usdjpy}")
            account_id = account_id_usdjpy
        ...
        if not account_id:
            logger.error("❌ OANDA_ACCOUNT_ID_USDJPY or OANDA_ACCOUNT_ID must be set in .env.practice")
            sys.exit(1)
        if not api_key:
        ...
        strategy = USDJPYMLEnsembleStrategy(config, oanda, news_avoidance=simple_news_avoidance)
    ...
    logger.info("=" * 70)
    logger.info("🛑 USD/JPY ML ENSEMBLE AUTO TRADER STOPPED")
    logger.info("=" * 70)
