...
class OANDAService:
    """Service to handle all OANDA API interactions for EUR/USD ML Ensemble Bot"""
    ...
    def place_market_order(self, instrument: str, units: float, stop_loss: float = None, take_profit: float = None, client_tag: str = None) -> Optional[dict]:
        """Place a market order with proper price formatting for EUR/USD (5 decimals)"""
        ...
        tag_value = client_tag or "EURUSD-ML-Ensemble"
        client_extensions = {
            "tag": tag_value,
            "comment": f"EUR/USD ML Ensemble Bot - {tag_value}"
        }
        ...
        # Fixed stop loss and take profit with proper formatting (5 decimals for EUR/USD)
        if stop_loss:
    ...
    def get_open_trades(self, instrument: str = "EUR_USD") -> Optional[List[dict]]:
    ...
    def calculate_position_size(self, account_balance: float, risk_percentage: float, stop_loss_pips: float) -> float:
        """Calculate position size based on risk for EUR/USD"""
        try:
            # Risk amount in account currency
            risk_amount = account_balance * (risk_percentage / 100)
            
            # For EUR/USD, pip value is $10 per 100,000 units (standard lot)
