...
from strategy_ml_ensemble import EURUSDMLEnsembleStrategy
...
UTC = pytz.UTC
log_file = log_dir / f"eurusd_ml_ensemble_{datetime.now(UTC).strftime('%Y%m%d')}.log"
...
def main():
    """Main bot loop"""
    logger.info("=" * 70)
    logger.info("🤖💶💵 EUR/USD ML ENSEMBLE AUTO TRADER STARTING")
    logger.info("=" * 70)
    ...
    try:
        ...
        account_id_eurusd = os.getenv('OANDA_ACCOUNT_ID_EURUSD')
        account_id_general = os.getenv('OANDA_ACCOUNT_ID')
        
        if account_id_eurusd:
            logger.info(f"📋 Found OANDA_ACCOUNT_ID_EURUSD: {account_id_eurusd}")
            account_id = account_id_eurusd
        ...
        if not account_id:
            logger.error("❌ OANDA_ACCOUNT_ID_EURUSD or OANDA_ACCOUNT_ID must be set in .env.live")
            sys.exit(1)
        if not api_key:
            logger.error("❌ OANDA_API_TOKEN or OANDA_API_KEY must be set in .env.live")
            sys.exit(1)
        ...
        strategy = EURUSDMLEnsembleStrategy(config, oanda, news_avoidance=simple_news_avoidance)
    ...
    logger.info("=" * 70)
    logger.info("🛑 EUR/USD ML ENSEMBLE AUTO TRADER STOPPED")
    logger.info("=" * 70)
