#!/usr/bin/env python3
"""
GBP/USD ML5 Ensemble Signal Service
...
UTC = pytz.UTC
log_file = log_dir / f"gbpusd_ml5_signal_service_{datetime.now(UTC).strftime('%Y%m%d')}.log"
...
def load_config():
    ...
    if config_file.exists():
        ...
        return {
            ...
            "description": "GBP/USD ML5 Ensemble Signal Service - Generates signals using 5-model XGBoost ensemble"
        }
...
def should_generate_new_signal(db: Session) -> bool:
    ...
    """
    try:
        # Check for most recent signal for GBP/USD
        stmt = select(MLSignalHistory).where(
            MLSignalHistory.instrument == "GBP_USD"
...
def main():
    ...
    logger.info("=" * 70)
    logger.info("📡💷💵 GBP/USD ML5 ENSEMBLE SIGNAL SERVICE STARTING")
    logger.info("=" * 70)
    
    try:
        # Load config
        config = load_config()
        instrument = config.get("instrument", "GBP_USD")
    ...
    logger.info("=" * 70)
    logger.info("🛑 GBP/USD ML5 ENSEMBLE SIGNAL SERVICE STOPPED")
    logger.info("=" * 70)
