...
UTC = pytz.UTC
log_file = log_dir / f"eurusd_ml5_signal_service_{datetime.now(UTC).strftime('%Y%m%d')}.log"
...
def load_config():
    ...
    if config_file.exists():
        ...
        return {
            ...
            "description": "EUR/USD ML5 Ensemble Signal Service - Generates signals using 5-model XGBoost ensemble"
        }
...
def should_generate_new_signal(db: Session) -> bool:
    ...
    """
    try:
        # Check for most recent signal for EUR/USD
...
def main():
    ...
    logger.info("=" * 70)
    logger.info("📡💶💵 EUR/USD ML5 ENSEMBLE SIGNAL SERVICE STARTING")
    logger.info("=" * 70)
    
    try:
        # Load config
        config = load_config()
        instrument = config.get("instrument", "EUR_USD")
    ...
    logger.info("=" * 70)
    logger.info("🛑 EUR/USD ML5 ENSEMBLE SIGNAL SERVICE STOPPED")
    logger.info("=" * 70)
